...
/**
 * Controller klassen för applikationen som sköter all logik i
 * Bibliotekssystemet.
 */
public class LibraryController {
	public Borrower currentBorrower;
	GUI theGUI;
	...
	/**
	 * Konstruktor som ser till att boota systemet med samtliga Media, Borrowers,
	 * Borrowed och MediaSearchResults i ArrayList
	 */
	...
	/**
	 * Konstruktor som ser till att boota systemet med samtliga Media, Borrowers,
	 * Borrowed och MediaSearchResults i ArrayList samt instantiera GUI fönstret
	 */
	 ...
	 * 
	 * @param inputString. Strängen som tas emot som input @return. True om det är
	 *                     korrekt dataformat, annars False.
	 */
	...
	/*
	 * Samma som ovan fast strängen får enbart innehålla siffro, används till
	 * Media-ObjectID
	 */
	...
	public boolean checkInputOnlyDigits(String inputString) {
		...
		String regex = "^-?\\d*?\\d+$";
		if (inputString == null) {
			theGUI.alertInfo("Incorrect characters only 0-9 are allowed");	
			return false;
		...
		try {
			if (Integer.parseInt(inputString) < Integer.MIN_VALUE) {
				theGUI.alertInfo("The number is too low, min is " + Integer.MIN_VALUE);	
				return false;
			}
		} catch (Exception e) {
			theGUI.alertInfo("The number is too high or low, only this is allowed: " + Integer.MIN_VALUE + " - " + Integer.MAX_VALUE);	
			return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param borrowerID. Låntagarens ID @return. True om denne existerar, annars
	 *                    false
	 */
	 ...
	 * 
	 * @param ID. Media ObjectID @return. Media objektet som söks. null om det
	 *            sökta objektet inte existerar
	 */
	 ...
	 * 
	 * @param theString. Texten på det sökta Media objektet @return. Media
	 *                   objektet som man vill ha detaljerat information om
	 */
	public void showSelectedMediaInfo(String theString) {
		...
		while (mediaIterator.hasNext()) {
			...
			if (tempMedia.toString().equals(theString)) {
				theGUI.alertInfo(tempMedia.listInfo());		        
	...
	/**
	 * Sök Media genom att skriva in valfritt sträng, jämför flera olika
	 * attribut.
	 * 
	...
	private void boot() {
		if (loadFileBorrowers() == false) {
			theGUI.alertInfo("files/Lantagare.txt not found");	
			
		}
		if (loadFileMedia() == false) {
			theGUI.alertInfo("files/Media.txt not found");	
		}
		if (loadBorrowedMedia() == false) {
			theGUI.alertInfo("files/Utlanade.txt not found");	
		}
	}
	...
	/**
	 * Läs in Media.text filen @return. True om filen existerar, annars False.
	 */
